...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            var worldResource = ReadString(args, "--world=", DefaultWorldResource);
            var worldIdentityProbe = WorldResourceIdentity3D.Create(worldResource, ReadOnlySpan<byte>.Empty);
            var worldPath = Path.Combine(
                AppContext.BaseDirectory,
                worldIdentityProbe.ResourcePath.Replace('/', Path.DirectorySeparatorChar));
            ...
            var server = new AuthoritativeServer3D(
                port,
                worldDefinition,
                worldIdentity);
